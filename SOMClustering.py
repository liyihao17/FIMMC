...
close_ngram = FrequentItemMining.get_close_ngram(text_type,'text',3,7,10,0.3)
...
dataset = np.matrix(data_l)
print(dataset)

dataset_old = dataset.copy()

som = SOM(dataset, (5, 5), 2, 119)
som.train()
...
for key in classify:
    tmp = []
    for i in range(len(classify[key])):
        tmp.append(text_type[classify[key][i]])
    result.append(tmp)
